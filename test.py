...
def parallel_image_loader(dataset: Dataset, num_workers: int = 8, image_shape=(256, 256), num_threads=256):
    ...
    with multiprocessing.Pool(num_workers) as pool:
        print(f"Starting parallel image loader with {num_workers} workers")
        iteration = 0
        while True:
            # Repeat forever
    ...
    def __init__(
        ...
        if isinstance(dataset, str):
            ...
            if isinstance(dataset[0], str):
                ...
                dataset = [load_from_disk(dataset_path) if "gs://" in dataset_path else load_dataset(dataset_path, split=default_split) for dataset_path in dataset]
...
if __name__ == "__main__":
    dataloader = OnlineStreamingDataLoader([
        ...
        ], batch_size=16, num_workers=64, num_threads=512, default_split="train")

    for i in tqdm.tqdm(range(0, 2000)):
        batch = next(dataloader)
        # print(f"Length of data queue: {data_queue.qsize()}, Length of error queue: {error_queue.qsize()}")
        # time.sleep(0.5)